...
void displayGraph(const std::set<VertexIndex>& picked_nodes_current, const std::set<VertexIndex>& picked_nbs_current, const MPGraphInterface& mpGraph, std::string fileName){
    displayGraphPNGNodesNbsPicked(mpGraph, 1000, 800, picked_nodes_current, picked_nbs_current, fileName);
}
...
GreedyWithDynPathsSol greedyDynPaths1dir(const MPGraphInterface& mpGraph, bool display_iterations){
    ...
    while(cont){
        ...
        if(picked.nodes.size()==0){
            cont = false;
//            std::cout<<"Picked nodes is size 0 at iteration " << iteration << std::endl;
            break;
        }
...
//            std::cout << "Current best solution: sum of weights: " << sum_of_weights_total << ", nr nbs: " << nr_nbs_total << " but nb set size: " << picked_nbs_current.size() << std::endl;
        }
