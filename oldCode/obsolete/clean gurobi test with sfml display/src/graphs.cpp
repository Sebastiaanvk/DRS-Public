...
int MultipartiteGraph::getLayers() const{
    return layers;
}

int MultipartiteGraph::getElementsInLayer(int layer) const {
    return elementsPerLayer.at(layer);
}
float MultipartiteGraph::getValue(const VertexIndex& v) const {
    return values.at(v);
}

MultipartiteGraph& MultipartiteGraph::operator=(const MultipartiteGraph& other){
    if (this != &other) {
        layers = other.layers;
